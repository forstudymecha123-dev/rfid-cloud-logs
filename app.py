"""
  HYDRA — RFID Attendance Intelligence  ≋ Water Edition ≋
  ─────────────────────────────────────────────────────────
  Stack  : Python · Firebase RTDB · firebase-admin · Streamlit
  Theme  : Deep Ocean — Abyssal blues, bioluminescent teal
  Build  : 3.1.0 — Water Edition
"""
...
st.set_page_config(
    page_title="Hydra — Water Intelligence",
    page_icon="≋",
    layout="wide",
...
:root {
  ...
  --bg-base:            #03080f;
  --bg-elevated:        #060d18;
  --bg-card:            rgba(14, 90, 140, 0.10);
  --bg-card-hover:      rgba(14, 90, 140, 0.18);
  --bg-input:           rgba(20, 100, 150, 0.12);

  --border-subtle:      rgba(0, 200, 255, 0.08);
  --border-moderate:    rgba(0, 200, 255, 0.16);
  --border-strong:      rgba(0, 200, 255, 0.35);

  --text-primary:       #d6f4ff;
  --text-secondary:     #5a9ab5;
  --text-tertiary:      #2d5a72;

  --accent:             #00c8e8;
  --accent-dim:         rgba(0, 200, 232, 0.12);
  --accent-glow:        0 0 20px rgba(0,200,232,0.18), 0 0 60px rgba(0,200,232,0.06);

  --status-ok:          #00e5b0;
  --status-ok-dim:      rgba(0, 229, 176, 0.10);
  --status-denied:      #ff4f6a;
  --status-denied-dim:  rgba(255, 79, 106, 0.10);
  --status-unknown:     #3a6a80;
  --status-unknown-dim: rgba(58, 106, 128, 0.15);
...
/* Animated deep-water background gradient */
[data-testid="stAppViewContainer"] {
  background:
    radial-gradient(ellipse 80% 50% at 20% 80%, rgba(0,80,140,0.18) 0%, transparent 60%),
    radial-gradient(ellipse 60% 40% at 80% 20%, rgba(0,140,180,0.10) 0%, transparent 55%),
    #03080f !important;
}
...
::-webkit-scrollbar-thumb { background: rgba(0,200,232,0.2); border-radius: 99px; }
...
/* Ripple pulse for live dot */
@keyframes ripple {
  0%   { box-shadow: 0 0 0 0 rgba(0,200,232,0.5); }
  70%  { box-shadow: 0 0 0 8px rgba(0,200,232,0); }
  100% { box-shadow: 0 0 0 0 rgba(0,200,232,0); }
}

/* Slow wave shimmer on brand */
...
.live-pill {
  ...
  border: 0.5px solid rgba(0,229,176,0.25); border-radius:99px;
  padding:0.28rem 0.75rem;
  font-size:0.68rem; font-weight:500; letter-spacing:0.08em; text-transform:uppercase;
  color: var(--status-ok);
}
.live-pill.offline { background:var(--status-unknown-dim); border-color:rgba(58,106,128,0.3); color:var(--text-secondary); }
.live-dot {
...
.stat-card::before {
  ...
  background:linear-gradient(90deg, transparent, rgba(0,200,232,0.35) 40%, rgba(0,200,232,0.35) 60%, transparent);
}
/* bottom water-line accent */
.stat-card::after {
  content:''; position:absolute; bottom:0; left:20%; right:20%; height:1px;
  background:linear-gradient(90deg, transparent, rgba(0,200,232,0.15), transparent);
}
.stat-icon { font-size:1rem; margin-bottom:0.9rem; opacity:0.45; }
...
.log-avatar {
  ...
  border: 0.5px solid rgba(0,200,232,0.18);
  color:var(--accent);
}
...
.pill-ok      { background:var(--status-ok-dim);      color:var(--status-ok);      border:0.5px solid rgba(0,229,176,0.25); }
.pill-ok::before { background:var(--status-ok); }
.pill-denied  { background:var(--status-denied-dim);  color:var(--status-denied);  border:0.5px solid rgba(255,79,106,0.25); }
.pill-denied::before { background:var(--status-denied); }
.pill-unknown { background:var(--status-unknown-dim); color:var(--status-unknown); border:0.5px solid rgba(58,106,128,0.3); }
...
.panel {
  ...
  border-radius:var(--radius-xl); padding:1.3rem 1.2rem;
  backdrop-filter:blur(28px) saturate(180%); -webkit-backdrop-filter:blur(28px) saturate(180%);
  margin-bottom:1rem; position:relative; overflow:hidden;
  animation: fade-up var(--dur-slow) var(--ease) 0.12s both;
}
.panel::before {
  content:''; position:absolute; top:0; left:12%; right:12%; height:0.5px;
  background:linear-gradient(90deg, transparent, rgba(0,200,232,0.2) 40%, rgba(0,200,232,0.2) 60%, transparent);
}

/* ── BAR CHART ── */
.bar-chart-wrap { display:flex; align-items:flex-end; gap:4px; height:68px; margin-top:0.9rem; padding-bottom:4px; }
.bar-col { flex:1; display:flex; flex-direction:column; align-items:center; gap:3px; }
.bar-fill { width:100%; border-radius:3px 3px 0 0; background:rgba(0,200,232,0.12); transform-origin:bottom; animation:bar-rise var(--dur-slow) var(--ease) both; min-height:3px; }
...
.alert-banner {
  ...
  background:rgba(0,200,232,0.04); border:0.5px solid var(--border-subtle);
  border-radius:var(--radius-md); padding:0.65rem 1rem; margin-bottom:2rem;
...
div[data-testid="stSelectbox"] label {
  font-family:var(--font) !important; font-size:0.66rem !important; font-weight:500 !important;
  letter-spacing:0.1em !important; text-transform:uppercase !important; color:var(--text-secondary) !important;
}
div[data-testid="stSelectbox"] > div > div {
  background:var(--bg-input) !important; border:0.5px solid var(--border-moderate) !important;
...
div[data-testid="stButton"] > button {
  width:100% !important; background:rgba(0,200,232,0.05) !important;
  border:0.5px solid var(--border-moderate) !important; color:var(--accent) !important;
  font-family:var(--font) !important; font-size:0.77rem !important; font-weight:500 !important;
  letter-spacing:0.06em !important; border-radius:var(--radius-sm) !important;
  padding:0.58rem 1rem !important; margin-top:0.5rem !important;
  transition:background var(--dur-fast) var(--ease), border-color var(--dur-fast) var(--ease) !important;
}
div[data-testid="stButton"] > button:hover { background:rgba(0,200,232,0.10) !important; border-color:var(--border-strong) !important; }
...
def fetch_logs_firebase(db_ref, limit=200):
    try:
        ...
        st.warning(f"Firebase error: {e}"); return pd.DataFrame()
...
def render_nav(is_live, ts):
    pc="live-pill" if is_live else "live-pill offline"
    pt="Live" if is_live else "Demo"
    st.markdown(f"""
    <div class="aura-nav">
      <div class="aura-brand">
        <span class="aura-brand-name">Hydra</span>
        <span class="aura-brand-sep">/</span>
        <span class="aura-brand-sub">RFID Attendance Intelligence · Water Edition</span>
      </div>
      <div class="aura-nav-right">
        <div class="{pc}"><div class="live-dot"></div>{pt}</div>
...
def render_stats(total, unique, uptime):
    st.markdown(f"""
    <div class="stat-row">
      <div class="stat-card"><div class="stat-icon">≋</div><div class="stat-label">Total Scans Today</div><div class="stat-value clr-accent">{total:,}</div><div class="stat-note">RFID events logged</div></div>
      <div class="stat-card"><div class="stat-icon">◎</div><div class="stat-label">Students Present</div><div class="stat-value clr-ok">{unique:,}</div><div class="stat-note">Unique identities verified</div></div>
      <div class="stat-card"><div class="stat-icon">∿</div><div class="stat-label">System Uptime</div><div class="stat-value" style="font-size:2rem;letter-spacing:-0.02em;">{uptime}</div><div class="stat-note">hh : mm : ss continuous</div></div>
    </div>""", unsafe_allow_html=True)
...
def main():
    ...
    st.markdown(f'<div class="aura-footer"><span class="footer-l">Hydra · Water Edition · Build 3.1.0</span><span class="footer-r">Last sync: {refresh_ts} UTC</span></div>', unsafe_allow_html=True)
